...
```{r}
params_run <- read.table(sprintf("%s%s_params_run.tab", log_dir, data_prefix), sep = "\t", quote = "", header=T)
sumstats_run <- read.table(sprintf("%s%s_sumstats_run.tab", log_dir, data_prefix), sep = "\t", quote = "", header=T)
sumstats_celltype <- read.table(sprintf("%s%s_sumstats_celltype.tab", log_dir, data_prefix), sep = "\t", quote = "", header=T)
...
vec_magma.fdr.log.path <- dir(path = tables_dir, pattern = "magma.fdr.log", full.names = T, recursive = F)
list_magma.fdr.log <- list()
vec_run_prefix <- sapply(vec_magma.fdr.log.path, function(x) gsub(paste0(tables_dir, "/", "|", data_prefix, "_","|_magma.fdr.log.csv"), "", x)) 
...
invisible(parLapply(cl, vec_run_prefix, function(run_pref) {
  ...
  ggsave(p, filename = paste0(plots_dir, data_prefix, "_", run_pref, "_magma.fdr.log.pdf") ,width=45,height=12)
  ...
  ggsave(p, filename = paste0(plots_dir, data_prefix, "_", run_pref,"_magma.fdr.log.signif.pdf") ,width=45,height=12)
...
vec_rareVariants.path <- dir(path = tables_dir, pattern = "mendelian_rareVariants_GSEA.csv", full.names = T, recursive = F)
list_rareVariants <- lapply(vec_rareVariants.path, function(x) read.csv(file=x, header=T, quote=""))
vec_run_prefix <- sapply(vec_rareVariants.path, function(x) gsub(paste0(tables_dir, "/", "|", data_prefix, "_","|_mendelian_rareVariants_GSEA.csv"), "", x)) 
...
for (run_pref in vec_run_prefix) {
  ...
  ggsave(p1, filename = paste0(plots_dir, data_prefix, "_", run_pref, "_rareVariants_mendelian.pdf") ,width=45,height=12)
}
...
vec_cellModEmbed_path <- dir(path = tables_dir, pattern=paste0(fuzzyModMembership, "_cellModEmbed"), full.names = T, ignore.case = T)
vec_run_prefix <- sapply(vec_cellModEmbed_path, function(x) gsub(paste0(tables_dir, "/", "|", data_prefix, "_","|_", fuzzyModMembership, "_cellModEmbed.csv"), "", x)) 
...
vec_ident_path <- sapply(vec_run_prefix, function(x) dir(path=scratch_dir, pattern=paste0(data_prefix, "_", x, "_ident"), full.names=T))
...
invisible(parLapply(cl, vec_run_prefix, function(i) {
  ...
  pdf(sprintf("%s%s_%s_%s_cellModEmbed_cellOrder.pdf", plots_dir, data_prefix, run_prefix, fuzzyModMembership),h=max(10, nrow(cellModEmbed_mat) %/% 1000), w=max(8, ncol(cellModEmbed_mat) %/% 2))
  draw(ht1+htra)
  dev.off()
  ...
  ht2 <- Heatmap(cellModEmbed_mat_sqrt[,], 
               ...
               heatmap_legend_param = list(title = "Expression"), use_raster = T)
               #top_annotation = eval(parse(text = paste0("list_htca_fdr", "[[", 1:length(list_htca_fdr),"]]"))))
  pdf(sprintf("%s%s_%s_%s_cellModEmbed_cellOrder_sqrt.pdf", plots_dir, data_prefix, run_prefix, fuzzyModMembership),h=max(10, nrow(cellModEmbed_mat) %/% 1000), w=max(8, ncol(cellModEmbed_mat) %/% 2))
  draw(ht2+htra)
  dev.off()
  ...
  ht3 <- Heatmap(cellModEmbed_mat[,], 
                 ...
                 heatmap_legend_param = list(title = "Expression"), use_raster = T)
                 #top_annotation = eval(parse(text = paste0("list_htca_fdr", "[[", 1:length(list_htca_fdr),"]]"))))
  pdf(sprintf("%s%s_%s_%s_cellModEmbed_cellCluster.pdf", plots_dir, data_prefix, run_prefix, fuzzyModMembership), h=max(10, nrow(cellModEmbed_mat) %/% 1000), w=max(8, ncol(cellModEmbed_mat) %/% 2))
  draw(ht3+htra)
  dev.off()
  ...
  ht4 <- Heatmap(cellModEmbed_mat_sqrt[,], 
                 ...
                 heatmap_legend_param = list(title = "Expression"), use_raster = T)
                 #top_annotation = eval(parse(text = paste0("list_htca_fdr", "[[", 1:length(list_htca_fdr),"]]"))))
  pdf(sprintf("%s%s_%s_%s_cellModEmbed_sqrt_cellCluster.pdf", plots_dir, data_prefix, run_prefix, fuzzyModMembership),h=max(10, nrow(cellModEmbed_mat) %/% 1000), w=max(8, ncol(cellModEmbed_mat) %/% 2))
  draw(ht4+htra)
  dev.off()
