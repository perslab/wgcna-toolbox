...
dir_project = "/projects/jonatan/tmp-epilepsy/"
prefix_data = "EP_UPF"
prefix_run = "1"
...
ipak("dplyr", "WGCNA", "Seurat", "ggplot2", "reshape", "reshape2", "parallel") # ComplexHeatmap? corrplot? RcolorBrewer?
...
```{r}
session_image_path <- dir(pattern = paste0(data_prefix, "_", run_prefix,"_final_session_image"), path = dir_RObjects, full.names = T)
...
```{r}
params_path <- dir(path=dir_log, pattern = paste0(prefix_data,"_params_run\\.tab"), full.names = T)
params_run <- read.table(params_path, sep = "\t", quote = "", header=T)
sumstats_run_path <- dir(path=dir_log, pattern = paste0(prefix_data,"_sumstats_run\\.tab"), full.names = T)
sumstats_run <- read.table(sumstats_run_path, sep = "\t", quote = "", header=T)
sumstats_celltype_path <- dir(path=dir_log, pattern = paste0(prefix_data,"_sumstats_celltype\\.tab"), full.names = T)
sumstats_celltype <- read.table(sumstats_celltype_path, sep = "\t", quote = "", header=T)
...
vec_magma.fdr.log.path <- dir(path = dir_tables, pattern = "magma.fdr.log", full.names = T, recursive = F)
list_magma.fdr.log <- list()
vec_run_prefix <- sapply(vec_magma.fdr.log.path, function(x) gsub(paste0(dir_tables, "/", "|", data_prefix, "_","|_magma.fdr.log.csv"), "", x)) 
names(vec_magma.fdr.log.path) <- vec_run_prefix

cl <- makeCluster(spec = 10, type = "FORK")

invisible(parLapply(cl, vec_run_prefix, function(run_pref) {

  magma.fdr.log <- read.csv(file=vec_magma.fdr.log.path[run_pref], header=T, quote="") 

  dat <- magma.fdr.log[,-grep("celltype", colnames(magma.fdr.log))]
  ...
  ggsave(p, filename = paste0(dir_plots, data_prefix, "_", run_pref, "_magma.fdr.log.pdf") ,width=45,height=12)

  magma.fdr.log.sig <- magma.fdr.log[, grep("module|celltype|BMI|scz|t1d|t2d", colnames(magma.fdr.log), ignore.case = T)]
  magma.fdr.log.sig <- magma.fdr.log.sig[apply(magma.fdr.log.sig[,-grep("module|celltype", colnames(magma.fdr.log.sig))], FUN = function(y) max(y) > -log10(p.val.threshold), MARGIN=1),]
  
  dat2 <- magma.fdr.log.sig[,-grep("celltype", colnames(magma.fdr.log.sig))]
  ...
  ggsave(p, filename = paste0(dir_plots, data_prefix, "_", run_pref,"_magma.fdr.log.signif.pdf") ,width=45,height=12)
...
vec_rareVariants.path <- dir(path = dir_tables, pattern = "mendelian_rareVariants_GSEA.csv", full.names = T, recursive = F)
list_rareVariants <- lapply(vec_rareVariants.path, function(x) read.csv(file=x, header=T, quote=""))
vec_run_prefix <- sapply(vec_rareVariants.path, function(x) gsub(paste0(dir_tables, "/", "|", data_prefix, "_","|_mendelian_rareVariants_GSEA.csv"), "", x)) 
# TODO: Error in FUN(X[[i]], ...) : object 'variable' not found
for (run_pref in vec_run_prefix) {

  dat1 <- list_rareVariants[[run_pref]][,-grep("celltype", colnames(list_rareVariants[[run_pref]]))]
  ...
  ggsave(p1, filename = paste0(dir_plots, data_prefix, "_", run_pref, "_rareVariants_mendelian.pdf") ,width=45,height=12)
}

# TODO only significant
list_rareVariants.sig <- lapply(list_rareVariants, function(x) x[apply(x[,-grep("module|celltype", colnames(x))], FUN = function(x) max(x) > -log10(p.val.threshold), MARGIN=1),])
...
ggsave(p2, filename = paste0(dir_plots, data_prefix, "_", "_rareVariants_signif_", flag_date, ".pdf") ,width=45,height=12)
...
#### Plot cell x eigengene embedding matrix for a single run
...
vec_cellModEmbed_path <- dir(path = dir_tables, pattern=paste0(fuzzyModMembership, "_cellModEmbed"), full.names = T, ignore.case = T)
vec_run_prefix <- sapply(vec_cellModEmbed_path, function(x) gsub(paste0(dir_tables, "/", "|", data_prefix, "_","|_", fuzzyModMembership, "_cellModEmbed.csv"), "", x)) 
list_cellModEmbed_mat <- lapply(vec_cellModEmbed_path, function(x) read.csv(file = x, header = T, quote = ""))
names(list_cellModEmbed_mat) <- vec_run_prefix
vec_ident_path <- sapply(vec_run_prefix, function(x) dir(path=dir_scratch, pattern=paste0(data_prefix, "_", x, "_ident"), full.names=T))

cl <- makeCluster(spec = 10, type = "FORK")

invisible(parLapply(cl, vec_run_prefix, function(i) {

  cellModEmbed_mat <- list_cellModEmbed_mat[[i]]
  ...
  #scale_data <- load_obj(f=sprintf("%s%s_scale_regr_data_ensembl_%s.RData", dir_scratch, data_prefix, flag_date))
  
  # scale_data_path <- dir(path = dir_scratch, pattern = "scale_regr_data_ensembl", full.names = T)
  # scale_data <- load_obj(scale_data_path) 
  
  #seurat_obj <- SetAllIdent(seurat_obj, id = "ClusterName")

  ident <- load_obj(f=vec_ident_path[i])
  ...
  ht2 <- Heatmap(cellModEmbed_mat_sqrt[,], 
               ...
               heatmap_legend_param = list(title = "Expression"), use_raster = T)
               #top_annotation = eval(parse(text = paste0("list_htca_fdr", "[[", 1:length(list_htca_fdr),"]]"))))
  pdf(sprintf("%s%s_%s_%s_cellModEmbed_cellOrder_sqrt.pdf", dir_plots, data_prefix, run_prefix, fuzzyModMembership),h=max(10, nrow(cellModEmbed_mat) %/% 1000), w=max(8, ncol(cellModEmbed_mat) %/% 2))
  draw(ht2+htra)
  dev.off()
  ...
  ht3 <- Heatmap(cellModEmbed_mat[,], 
                 ...
                 heatmap_legend_param = list(title = "Expression"), use_raster = T)
                 #top_annotation = eval(parse(text = paste0("list_htca_fdr", "[[", 1:length(list_htca_fdr),"]]"))))
  pdf(sprintf("%s%s_%s_%s_cellModEmbed_cellCluster.pdf", dir_plots, data_prefix, run_prefix, fuzzyModMembership), h=max(10, nrow(cellModEmbed_mat) %/% 1000), w=max(8, ncol(cellModEmbed_mat) %/% 2))
  draw(ht3+htra)
  dev.off()
  ...
  ht4 <- Heatmap(cellModEmbed_mat_sqrt[,], 
                 ...
                 heatmap_legend_param = list(title = "Expression"), use_raster = T)
                 #top_annotation = eval(parse(text = paste0("list_htca_fdr", "[[", 1:length(list_htca_fdr),"]]"))))
  pdf(sprintf("%s%s_%s_%s_cellModEmbed_sqrt_cellCluster.pdf", dir_plots, data_prefix, run_prefix, fuzzyModMembership),h=max(10, nrow(cellModEmbed_mat) %/% 1000), w=max(8, ncol(cellModEmbed_mat) %/% 2))
  draw(ht4+htra)
  dev.off()
...
#                heatmap_legend_param = list(title = "Expression"), use_raster = T)
#                #top_annotation = eval(parse(text = paste0("list_htca_fdr", "[[", 1:length(list_htca_fdr),"]]"))))
# pdf(sprintf("%s%s_%s_cellModEmbed_RowColOrder_%s.pdf", dir_plots, data_prefix, fuzzyModMembership,  flag_date ),h=max(10, nrow(cellModEmbed_mat) %/% 1000), w=max(8, ncol(cellModEmbed_mat) %/% 2))
# draw(ht5+htra)
# dev.off()  
# 
# 
...
#                heatmap_legend_param = list(title = "Expression"), use_raster = T)
#                #top_annotation = eval(parse(text = paste0("list_htca_fdr", "[[", 1:length(list_htca_fdr),"]]"))))
# pdf(sprintf("%s%s_%s_cellModEmbed_sqrt_RowColOrder_%s.pdf", dir_plots, data_prefix, fuzzyModMembership, flag_date ),h=max(10, nrow(cellModEmbed_mat) %/% 1000), w=max(8, ncol(cellModEmbed_mat) %/% 2))
# draw(ht6+htra)
# dev.off()  
```
...
pdf(sprintf("%s%s_mod_corr_%s.pdf", dir_plots, data_prefix, flag_date), width=max(8,ncol(cellModEmbed_mat) %/% 3),height=max(8,ncol(cellModEmbed_mat) %/% 3))
...
pdf(sprintf("%s%s_module_cluster_%s.pdf", dir_plots, data_prefix, flag_date),width=max(8,ncol(cellModEmbed_mat) %/% 3),height=max(8, ncol(cellModEmbed_mat) %/% 3))
...
```{r}
genelist_path <- dir(pattern = paste0(data_prefix, "_list_list_module_genes"), path = dir_RObjects, full.names = T)
...
scale_data_path <- dir(path = dir_scratch, pattern = paste0(data_prefix, "_scale_regr_data_ensembl"), full.names = T)
...
cl <- makeCluster(n_cores, type="FORK", outfile = paste0(dir_log, "merged_mod_cell_embed_mat.txt"))
...
if (!is.null(metadata)) {
  for (cell in names(list_mod_metadata_corr_rho)){
    pdf(sprintf("%s%s_%s_mod_corr_metadata_%s.pdf", dir_plots, data_prefix, cell, flag_date), w=max(8, ncol(list_mod_metadata_corr_rho[[cell]]) %/% 2), h=max(8, nrow(list_mod_metadata_corr_rho[[cell]]) %/% 2)) # TODO: adapt this dynamically
...
```{r, fig.height=10, fig.width=10}
#seurat_obj <- load_obj(f=sprintf("%sseurat_obj_ensembl.RData",dir_RObjects))
...
ggsave(paste0(dir_plots,data_prefix, "_TSNE_cell_clusters_", flag_date, ".pdf"),w=12, h=10)
...
for (feat in names(seurat_obj@meta.data)) {
  ...
  ggsave(paste0(dir_plots,data_prefix,"_featplot_", feat, flag_date, ".pdf"),w=12, h=10)
}
...
for (mod in names(seurat_obj@meta.data)) {
  ...
  ggsave(paste0(dir_plots,flag_date,"_",data_prefix,"_featplot_mod_", mod, ".pdf"),w=12, h=10)
}
