...
project_dir = "/projects/timshel/sc-genetics/sc-genetics/out/out.wgcna/maca_top29_BMI/"
data_prefix = "maca_top29_BMI"
...
ggsave(p, filename = paste0(plots_dir, data_prefix, "_magma_", flag_date, ".pdf") ,width=45,height=12)
...
cellModEmbed_mat <- cellModEmbed_mat[match(rownames(cellModEmbedorder(ident), ] 

# Take sqrt of values (first make positive, then sqrt, then restore original sign)
...
ht1 <- Heatmap(cellModEmbed_mat_sqrt[,], 
             cluster_rows = F,
             #row_order = ,
             cluster_columns = T, 
             #show_row_dend = F, 
...
pdf(sprintf("%s%s_cellModEmbed_sqrt_cellOrder_%s.pdf", plots_dir, data_prefix, flag_date ),h=20,w=20)
draw(ht1+htra)
dev.off()
...
ht2 <- Heatmap(cellModEmbed_mat_sqrt[,], 
               ...
               heatmap_legend_param = list(title = "Expression"))
               #top_annotation = eval(parse(text = paste0("list_htca_fdr", "[[", 1:length(list_htca_fdr),"]]"))))
pdf(sprintf("%s%s_cellModEmbed_sqrt_cellCluster_%s.pdf", plots_dir, data_prefix, flag_date ),h=20,w=20)
draw(ht2+htra)
dev.off()


# Without clustering rows nor columns
ht3 <- Heatmap(cellModEmbed_mat_sqrt[,], 
               cluster_rows = F,
               #row_order = NULL,
               cluster_columns = F, 
               #show_row_dend = F, 
               ...
               heatmap_legend_param = list(title = "Expression"))
               #top_annotation = eval(parse(text = paste0("list_htca_fdr", "[[", 1:length(list_htca_fdr),"]]"))))
pdf(sprintf("%s%s_cellModEmbed_sqrt_RowColOrder_%s.pdf", plots_dir, data_prefix, flag_date ),h=20,w=20)
draw(ht3+htra)
dev.off()  
```
