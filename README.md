...
* `minClusterSize`: Minimum genes needed to form a module. Takes a vector in quotes  with one or more values, without whitespace, e.g. `"c(15,20)"`. Recommended range 5-25. Defaults to `"c(15)"`
* `deepSplit`: Controls the sensitivity of the `cutreeDynamic` algorithm. Takes a vector in quotes with one or more values, without whitespace,  e.g. `"c(2,3)"`. Takes integer values 0-4, defaults to `"c(2)"`. 
* `moduleMergeCutHeight`: Cut-off level for the variable (1-correlation) for merging eigengenes. Takes a vector with one or more values, without whitespace, the vector should be in quotes, e.g. `"c(0.20,0.25)"`. Recommended value range 0.15-0.25
* `pamStage`: For `cutreeHybrid`. Perform additional Partition Around Medroids step? Takes a character, in single (double) quotes,  with a vector with one or two unquoted boolean values without whitespace, e.g. `"c(TRUE,FALSE)"`, default `"c(TRUE)"`
