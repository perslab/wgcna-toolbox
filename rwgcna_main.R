...
if (resume == "checkpoint_2") {
  ...
  if (fuzzyModMembership=="kME") {
    ...
    list_list_colors <- clusterMap(cl, function(list_merged,datExpr,cellType) {
    ...
    list_list_colors <- clusterMap(cl, function(x,y) name_for_vec(to_be_named=x, given_names = as.character(y), dimension=NULL),
                               x = list_list_colors,
  ...
  rm(list_datExpr, list_datExpr_gg, list_geneTree, list_list_cutree, list_list_MEs, list_list_plot_label)#, list_idx_mapped_gg)
...
if (resume == "checkpoint_4") {
  ...
  sumstats_celltype_df <- data.frame(run_prefix = rep(run_prefix, times=length(sNames)),
                                     ...
                                     prop_genes_mapped_to_ortholog = if (!is.null(magma_gwas_dir)) if (data_organism=="mmusculus") ifelse(test=sNames %in% sNames_PPImes_PPI, yes=vec_MMtoHsmapping_prop.mapped, no=NA) else rep(NA, length(sNames)) else rep(NA, length(sNames)), 
