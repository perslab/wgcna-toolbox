...
option_list <- list(
  ...
  make_option("--scale_data", type="logical", default=TRUE,
              help="Use scaled and centered data? In scaled data ribosomal and mitochrondrial genes are regressed out"),
  ...
  make_option("--STRINGdb_species", type="integer", default=NULL,
...
source(file = "/projects/jonatan/functions-src/wgcna_params_0.5.R")
...
for (subsetName in names(subsets)) {
  ...
  vars.to.regress = c("nUMI", "percent.mito", "percent.ribo")[c("nUMI","percent.mito", "percent.ribo") %in% names(subsets[[subsetName]]@meta.data)]
  
  if (!all(vars.to.regress %in% c("nUMI", "percent.mito", "percent.ribo"))) warning("One or more of nUMI, percent.mito, and percent.ribo not found in Seurat object meta data")
  
  seurat_obj_sub <- ScaleData(object = subsets[[subsetName]], 
  ...
  if (grepl("PCA", genes_use, ignore.case = T)) {
    seurat_obj_sub <- RunPCA(object = seurat_obj_sub,
                             ...
                             pcs.compute = min(nPC_seurat, length(seurat_obj_sub@var.genes)-1, ncol(seurat_obj_sub@data)-1),
                             do.print = F)
